...
(()=>{
  ...
  function show(){
    ...
    for(const cut of board.cuts){const d=document.createElement('details'),s=document.createElement('summary'),p=document.createElement('pre');s.textContent=`${cut.cut_number} (${cut.id}) ${cut.start_sec}–${cut.end_sec}s`;p.style.whiteSpace='pre-wrap';d.addEventListener('toggle',()=>{if(d.open&&!p.textContent)p.textContent=JSON.stringify({cut,current_analysis_references:(Array.isArray(cut.references)?cut.references:[]).map(r=>({id:r.target_id,value:MVStoryboard.snapshot(analysis,r.target_id)}))},null,2);});d.append(s,p);output.append(d);}
  }
